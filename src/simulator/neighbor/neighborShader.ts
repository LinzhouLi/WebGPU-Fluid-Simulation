...
@group(0) @binding(4) var<storage, read_write> particleSortIndexSrc: array<u32>;
@group(0) @binding(5) var<storage, read_write> particleSortIndexDest: array<u32>;
...
fn main( @builtin(global_invocation_id) global_id: vec3<u32> ) {
  ...
  particleSortIndexSrc[particleIndex] = atomicAdd(&(cellParticleCount[cellIndex]), 1u);
  return;
}
`;
...
fn main( @builtin(global_invocation_id) global_id: vec3<u32> ) {
  ...
  let cellCoord = vec3<u32>(floor(particlePosition[particleIndex] * vec3<f32>(grid.dimension)));
  let cellIndex = dot(cellCoord, grid.coord2index);
  let sortIndex = particleSortIndexSrc[particleIndex] + cellOffset[cellIndex];
	particleSortIndexDest[sortIndex] = particleIndex;
  return;
}
`;
...
const NeighborSearch = /* wgsl */`
  ...
  if ( all(nCoord >= vec3<i32>(0)) && all(nCoord < GridDim) ) {
    ...
    nParticleSortedIdx = cellOffset[nCellIdx];
    for (
      nCellParticleIdx = 0; 
      nCellParticleIdx < nCellParticleCount;
      nCellParticleIdx++
    ) {
      nParticleIdx = particleSortIndexDest[nParticleSortedIdx];
...
fn main( @builtin(global_invocation_id) global_id: vec3<u32> ) {
  ...
  var nParticleSortedIdx = u32();   // sorted index of a neighbor particle
  var nParticleIdx = u32();         // index of a neighbor particle
