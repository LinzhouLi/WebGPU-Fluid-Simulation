...
class Controller {
  ...
  private ifFluidSurface: boolean;
  ...
  public async initScene(camera: THREE.PerspectiveCamera, light: THREE.DirectionalLight) {
    ...
    await this.particleRenderer.initResource(this.globalResource.bindgroupLayout);

    // screen space fluid renderer
    this.fluidRenderer = new FluidRenderer(this.simulator, this.globalResource);
    await this.fluidRenderer.initResource(camera);
    this.config.initRenderOptions((e) => this.setRenderConfig(e.object));
    this.setRenderConfig(this.config.renderOptions);
  ...
  private setRenderConfig(options: Config['renderOptions']) {

    this.ifFluidSurface = options.renderMode === 0;
    this.fluidRenderer.setConfig(options);
  ...
  public run() {
    ...
    if(this.ifParticles && !this.ifFluidSurface) this.particleRenderer.render(renderPassEncoder);
    renderPassEncoder.end();

    // screen space fluid: the imposter passes rasterize against the scene depth
    // written above, the composite pass blends the surface over the scene
    if(this.ifParticles && this.ifFluidSurface)
      this.fluidRenderer.render(commandEncoder, ctxTextureView);
