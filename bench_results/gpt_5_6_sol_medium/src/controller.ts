...
class Controller {
  ...
  public run() {
    ...
    // Generate billboard depth/thickness and run the narrow-range filter
    // before compositing the reconstructed surface into the scene.
    if(this.ifParticles) this.particleRenderer.prepare(commandEncoder);
