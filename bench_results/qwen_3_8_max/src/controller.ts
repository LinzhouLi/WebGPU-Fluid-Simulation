...
import { FluidRenderer } from './renderer/fluid/fluidRenderer';
...
class Controller {
  ...
  private fluidParticleDepthView: GPUTextureView;
  ...
  private RegisterResourceFormats() {
    ...
    FluidRenderer.RegisterResourceFormats();
  }
  ...
  private setSceneConfig(config: {
    ...
    this.ifFluidRender = config.object.renderMode == 0;
  ...
  public async initScene(camera: THREE.PerspectiveCamera, light: THREE.DirectionalLight) {
    ...
    await this.fluidRenderer.initResource(this.globalResource);
    this.fluidParticleDepthView = this.fluidRenderer.particleDepthMap.createView();
    this.config.initFluidRenderOptions(
      (e) => this.fluidRenderer.setFilterParams(e.object)
    );
    this.fluidRenderer.setFilterParams(this.config.fluidRenderOptions);
  ...
  public run() {
    ...
    if(this.ifParticles && !this.ifFluidRender) this.particleRenderer.render(renderPassEncoder);
    renderPassEncoder.end();

    if (this.ifParticles && this.ifFluidRender) {
      this.renderFluid(commandEncoder, ctxTextureView);
    }
  ...
  private renderFluid(commandEncoder: GPUCommandEncoder, ctxTextureView: GPUTextureView) {

    // 1. billboard particle depth prepass
    const prepassEncoder = commandEncoder.beginRenderPass({
      colorAttachments: [],
      depthStencilAttachment: {
        view: this.fluidParticleDepthView,
        depthClearValue: 0.0,
        depthLoadOp: 'clear',
        depthStoreOp: 'store',
      }
    });
    this.globalResource.setResource(prepassEncoder);
    this.fluidRenderer.renderPrepass(prepassEncoder);
    prepassEncoder.end();

    // 2. narrow-range filter chain (convert + 1D H/V + clean-up)
    const finalIsA = this.fluidRenderer.runFilter(commandEncoder);

    // 3. screen-space composite into the scene image
    const compositeEncoder = commandEncoder.beginRenderPass({
      colorAttachments: [{
        view: ctxTextureView,
        loadOp: 'load',
        storeOp: 'store'
    ...
    this.globalResource.setResource(compositeEncoder);
    this.fluidRenderer.composite(compositeEncoder, finalIsA);
