...
class Config {
  ...
  public fluidRenderOptions = {
    sigma: 0.7,  // filter kernel size (in particle radii)
    delta: 10,   // narrow range (in particle radii)
    mu: 1,       // clamping distance (in particle radii)
    iterations: 2
  };
  ...
  public initSceneOptions(onChangeFunc: (msg) => void, switchFunc: () => void) {
    ...
    sceneOptionGUI.add(this.scnenOptions, 'renderMode',
      { 'Fluid Screen-Space': 0, 'Raw Particles': 1 }
    );
  ...
  public initFluidRenderOptions(onChangeFunc: (msg) => void) {

    const fluidOptionGUI = this.gui.addFolder('Fluid Render Options');
    fluidOptionGUI.add(this.fluidRenderOptions, 'sigma', 0.1, 2);
    fluidOptionGUI.add(this.fluidRenderOptions, 'delta', 1, 20);
    fluidOptionGUI.add(this.fluidRenderOptions, 'mu', 0, 10);
    fluidOptionGUI.add(this.fluidRenderOptions, 'iterations', 1, 3).step(1);
    fluidOptionGUI.onFinishChange(onChangeFunc);
